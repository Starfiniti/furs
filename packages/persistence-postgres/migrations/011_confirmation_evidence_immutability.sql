...
begin
  ...
  if old.certificate_fingerprint_sha256 is not null
    and old.certificate_fingerprint_sha256 is distinct from new.certificate_fingerprint_sha256 then
    raise exception using errcode = '55000', message = 'confirmation certificate fingerprint cannot be changed';
  end if;
  if old.confirmed_at is not null and old.confirmed_at is distinct from new.confirmed_at then
    raise exception using errcode = '55000', message = 'confirmation time cannot be changed';
  end if;
